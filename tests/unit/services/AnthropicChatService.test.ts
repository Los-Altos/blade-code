...
describe('AnthropicChatService', () => {
  ...
  describe('响应解析', () => {
    ...
    it('应正确处理多个 tool_use blocks', async () => {
      const service = new AnthropicChatService(baseConfig);
  ...
  describe('updateConfig', () => {
    it('应更新配置并重建客户端', () => {
      ...
      expect(mockAnthropic.instances).toHaveLength(2);
  ...
  describe('错误处理', () => {
    it('API 错误应向上抛出', async () => {
      ...
      mockAnthropic.createSpy.mockRejectedValue(error);
