...
export class IdeClient {
      ...
      switch (message.method) {
        ...
        case 'generateCode':
          result = await this.agent.chat(
            `Generate code: ${JSON.stringify(message.params)}`
          );
          break;

        case 'executeTool':
          result = await this.agent.chat(
            `Execute tool: ${JSON.stringify(message.params)}`
          );
          break;

        case 'analyzeFiles':
          result = await this.agent.chat(
            `Analyze files: ${JSON.stringify(message.params)}`
          );
          break;
  ...
  public async sendDiagnosticUpdate(
    filePath: string,
