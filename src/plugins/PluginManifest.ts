...
/**
 * Get the manifest directory path for a plugin
 *
 * @param pluginDir - Plugin root directory
 * @returns Path to the manifest directory, or null if not found
 */
export async function getManifestDir(
  pluginDir: string
): Promise<{ path: string; source: ManifestSource } | null> {
  for (const { dir, source } of MANIFEST_DIRS) {
    const manifestPath = path.join(pluginDir, dir, 'plugin.json');
    try {
      await fs.access(manifestPath);
      return { path: path.join(pluginDir, dir), source };
    } catch {
      continue;
    }
  }
 ...
 * @param preferBlade - If true, create in .blade-plugin/, otherwise .claude-plugin/
...
export async function createPluginManifest(
  ...
  const dir = preferBlade ? '.blade-plugin' : '.claude-plugin';
  const manifestDir = path.join(pluginDir, dir);
  ...
  logger.debug(`Created plugin manifest at ${manifestPath}`);
...
export async function updatePluginManifest(
  ...
  // Validate
  const result = pluginManifestSchema.safeParse(updated);
  if (!result.success) {
    const errors = result.error.issues
      .map((issue) => `${issue.path.join('.')}: ${issue.message}`)
      .join('; ');
    throw new Error(`Invalid manifest after update: ${errors}`);
  }
 ...
 * @returns Default manifest object
 */
